...
def bertscore_evaluation(df, test_set_pattern):
    from bert_score import score
    df = df.loc[df.pred != '']
    df = df.loc[~df.pred.isna()]
    
    def get_bertscore(pred: str, label: str):
        p, r, f1 = score(
            pred,
            label,
    ...
    for i, d in df.iterrows():
        answer = d['pred'].replace("\n", "\\n")
        p, r, f1 = get_bertscore([d['ground_truth']], [answer])
        p_lst.append(p)
        r_lst.append(r)
        f1_lst.append(f1)
    df['bertscore_p'] = p_lst
    df['bertscore_r'] = r_lst
    df['bertscore_f1'] = f1_lst    
        
    df_test = df.loc[df.id.str.contains(test_set_pattern)]
    print("Data points: ", df.shape[0])
    print("Avg BERTScore P:", df.bertscore_p.mean())
    print("Avg BERTScore R:", df.bertscore_r.mean())
    print("Avg BERTScore F1:", df.bertscore_f1.mean())
    return df
...
def main(file_path, dataset, amr_cot):
    ...
    if dataset in ['paws']:
        ...
        df = bertscore_evaluation(df, 'newstest16')
    elif dataset in ['django']:
...
if __name__ == '__main__':
    ...
    # model_list = ['text-davinci-001', 'text-davinci-002', 'text-davinci-003', 'gpt-4-0613', 'gpt-3.5-turbo-0613']
    model_list = ['text-davinci-003']
    set_seed(0)
    for model_version in model_list:
        main(f"/home/jiaruil5/amr/tmp/data/outputs/{model_version}/requests_amr_newstest.csv", 'newstest', True)
        # main(f"/home/jiaruil5/amr/tmp/data/outputs/{model_version}/requests_direct_newstest.csv", 'newstest', False)
