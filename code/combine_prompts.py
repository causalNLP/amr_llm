...
for filename in os.listdir(out_dir/'gpt-4-0613'):
    # if filename.endswith('.csv') and filename.replace("requests_amr_","") .replace("requests_direct_","").replace(".csv","") in dataset:
      if filename.endswith('.csv') and 'pubmed' in filename and not filename.startswith("."):
...
df_amr[['id','raw_prompt','pred']].to_csv(data_dir/'prompts_pubmed_amr.csv',index=False)

#save column 'raw_prompt' to a txt file, separate by '\n'
# replace more than one "\s" with one " "

with open(data_dir/'prompts_pubmed_direct.txt','w') as f:
    f.write('\n'.join(df_direct['raw_prompt'].astype(str).apply(
        lambda x: re.sub(' +', ' ', x.replace("\n", " ").replace("\t", " "))).tolist()))
with open(data_dir/'prompts_pubmed_amr.txt','w') as f:
    f.write('\n'.join(df_amr['raw_prompt'].astype(str).apply(
        lambda x: re.sub(' +', ' ', x.replace("\n", " ").replace("\t", " "))).tolist()))
